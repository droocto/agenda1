<?php
    ...
    $rootPath = ((@$_SERVER['DOCUMENT_ROOT'] && file_exists(@$_SERVER['DOCUMENT_ROOT'] . $_SERVER['PHP_SELF'])) ? $_SERVER['DOCUMENT_ROOT'] : str_replace(dirname(@$_SERVER['PHP_SELF']), '', str_replace('\\', '/', realpath('.'))));
    $realPath = realpath(dirname(__FILE__) . '/../../../../../site/webimg/').'/';
    $path['gallery'] = str_replace($rootPath, '/', $realPath);
...
/*		array (
	...
	$cfg['ilibs'] = array (																// image library path with slashes; absolute to root directory - please make sure that the directories have write permissions	
		array (																		
			'value'   	=> $path['gallery'],												
			'text'    	=> 'Galer&iacute;a de Im&aacute;genes',
		),
	...
	$cfg['ilibs_dir'] 	   = array($path['gallery']);						   	// image library path with slashes; absolute to root directory - please make sure that the directories have write permissions
	...
	$cfg['ver'] 		= '1.3.7';										// iBrowser version	
